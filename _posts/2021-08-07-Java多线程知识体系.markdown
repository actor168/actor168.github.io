...
`Java`内置关键字，通过`monitorenter`和`monitorexit`实现该能力，其实是对对象加锁。
具有原子性、可见性、可重入性、重量级锁等特点。
...
- 偏向锁、轻量级锁、重量级锁概念
偏向锁：介于无锁和轻量级锁之间的一种状态，由markword记录当前所占用对象的thread id
...
`Lock`, `ReentrantLock`, `Condition`, `ReentrantReadWriteLock`
